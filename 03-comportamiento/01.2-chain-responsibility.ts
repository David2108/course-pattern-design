...
class Supervisor extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    ...
    console.log(`%cSupervisor no puede aprobar la solicitud de $${amount}, pasando al siguiente aprobador...`, COLORS.red);
    super.next(amount);
  }
}
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if(amount <= 5000) {
      console.log(`%cManager aprobó la solicitud de $${amount}.`, COLORS.blue);
      return;
    }
    console.log(`%cManager no puede aprobar la solicitud de $${amount}, pasando al siguiente aprobador...`, COLORS.red);
    super.next(amount);
  }
}
...
class Director extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    console.log(`%cDirector aprobó la solicitud de $${amount}.`, COLORS.purple);
  }
}
